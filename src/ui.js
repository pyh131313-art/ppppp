...
function buildShopEmbed(playerInput, message = "商店限定紀念幣只能用金幣購買，挖礦、鑄造、除鏽都不會出。", progressInput = {}) {
  ...
  const taiwanToday = new Intl.DateTimeFormat("en-CA", {
    ...
    month: "2-digit",
    day: "2-digit"
  }).format(new Date(progressInput.now || Date.now()));
  ...
  const consumableLines = getShopConsumables(progressInput).map((item) => {
    ...
    if (item.id === "undyingTotem") {
      const bought = player.lastTotemResetDate === taiwanToday ? player.dailyTotemPurchaseCount || 0 : 0;
      return `${item.label}：${item.priceGold} 金幣\n今日剩餘：${Math.max(0, item.dailyLimit - bought)} / ${item.dailyLimit}｜持有 ${owned}`;
    }
    return `${item.label}｜${item.priceGold} 金幣｜持有 ${owned}`;
  });
  const miningItems = consumableLines.filter((line) => /治療藥水|不死圖騰/.test(line));
  const chickenItems = consumableLines.filter((line) => !/治療藥水|不死圖騰/.test(line));
  ...
  return new EmbedBuilder()
    ...
    .setDescription([
      ...
      "紀念幣",
      ...lines,
      ...(miningItems.length ? ["", "⛏️ 挖礦用品", ...miningItems] : []),
      ...(chickenItems.length ? ["", "🐔 養雞用品", ...chickenItems] : []),
      "",
      "微光池",
      "投入 400 金幣和 2 枚自選紀念幣，融合成 1 枚隨機紀念幣。",
      "",
      "供需行情",
...
function buildShopComponents(progressInput = {}, playerInput = null, targetUserId = null) {
  ...
  const chickenItemRow = [
    ...
    makeButton(`${CUSTOM_IDS.shopBuyPrefix}:chickenMedicine:1`, "特效藥", ButtonStyle.Success, "💊"),
    makeButton(`${CUSTOM_IDS.shopBuyPrefix}:autoCleaner:1`, "掃大便機", ButtonStyle.Success, "🤖")
  ];
  rows.push(new ActionRowBuilder().addComponents(...chickenItemRow));
...
function buildUndergroundInnComponents(snapshot = null) {
  ...
  const itemButtons = items.slice(0, 5).map((item) => {
    ...
    const emoji = firstSpace > 0 ? item.label.slice(0, firstSpace) : "🏨";
    const label = firstSpace > 0 ? item.label.slice(firstSpace + 1) : item.label;
    return makeButton(`${CUSTOM_IDS.undergroundInnBuyPrefix}:${item.id}`, label, ButtonStyle.Danger, emoji);
  });
  const rows = [];
  if (itemButtons.length) rows.push(new ActionRowBuilder().addComponents(...itemButtons));
  rows.push(new ActionRowBuilder().addComponents(
    makeButton(CUSTOM_IDS.undergroundInnBlessings, "查看祝福", ButtonStyle.Secondary, "📈"),
    makeButton(CUSTOM_IDS.undergroundCamp, "離開", ButtonStyle.Secondary, "↩️")
  ));
  return rows;
}

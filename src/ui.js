...
function buildCollectionFiles(playerInput) {
  return getCollectionImageCards(playerInput).map((card) => (
...
function buildCollectionEmbed(playerInput, message = "這是你的收藏紀念幣圖鑑。") {
  ...
  const capacity = getBagCapacity(player);
  const imageCount = getCollectionImageCards(player).length;
  const embed = new EmbedBuilder()
    .setColor(0x8b5cf6)
    .setTitle("紀念幣包包")
    .setDescription([
      ...
      `圖片顯示：${imageCount ? `下方 ${imageCount} 張` : "尚無可顯示圖片"}`
    ].join("\n"))
    .addFields(
...
function buildCollectionEmbeds(playerInput, message = "這是你的收藏紀念幣圖鑑。") {
  const mainEmbed = buildCollectionEmbed(playerInput, message);
  const imageEmbeds = getCollectionImageCards(playerInput).map((card) => (
    new EmbedBuilder()
      .setColor(0x8b5cf6)
      .setTitle(card.item.name)
      .setDescription(`${card.item.rarity}｜持有 x${card.count}`)
